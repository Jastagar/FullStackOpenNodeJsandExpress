...
mongoose.connect(process.env.DATABASE_URL)
    ...
    .catch((err) => {
        console.log('Could not Connect to mongo Database because of :', err)
    })
// Request handling.;
...
app.post('/api/persons', (req, res, next) => {
    ...
    Person.find({ name }, { number })
        .then((foundPerson) => {
            if (foundPerson.length) {
                console.log(foundPerson)
                res.send('Sorry this name or number already exists at id: ', foundPerson.id)
            } else {
